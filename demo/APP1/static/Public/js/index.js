...
$(function(){
    ...
    $("input.sum").bind("input propertychange",function(){       
        ...
        if($("input.sum").val()> parseInt(count.getAttribute("data-value"))){   
            $(this).val(count.getAttribute("data-value"));
            alert("商品数量超限！");
        };
    });
    ...
    $add.click(function(){
        ...
        if($inputSum.val()>=parseInt(count.getAttribute("data-value"))){
            $inputSum.val(count.getAttribute("data-value")-1);
            alert("商品数量超限！");
        }
    });
})
